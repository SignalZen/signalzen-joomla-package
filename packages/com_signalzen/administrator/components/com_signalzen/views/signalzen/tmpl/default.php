<?php
/**
 * @package Signalzen
 * @author SignalZen
 * @copyright (C) 2021 - SignalZen
 * @license GNU/GPLv3 http://www.gnu.org/licenses/gpl-3.0.html
**/

defined('_JEXEC') or die('Restricted Access');
?>

<div class="wrap">
  <h2>SignalZen Integration</h2>
  <p>
    <h3>Introduction</h3>
    SignalZen is a live chat solution for your website which enables you to engage and talk to your website visitors via Slack.<br />
    We provide you with simplicity oriented service which includes such features as <a href="https://slack.com" target="_blank">Slack</a> integration and <a href="http://docs.signalzen.com/" target="_blank">API</a> to work with your accumulated data sent by your website visitors.
    <br />
    Please read more at <a href="https://signalzen.com" target="_blank">signalzen.com</a>.
    <br />
    <br />
    <h3>Integration with your website</h3>
    This Joomla component should make the integration with SignalZen incredibly easy and fast.<br />
    However, before proceeding, make sure you have <a href="https://console.signalzen.com/sign-up" target="_blank">setup an account</a>.
    <br />
    <br />
    To enable SignalZen chat widget on your website, you just need to enter your <strong>Public Token</strong> which you can get in the <strong>Integration page</strong> while logged in to your account at <a href="https://signalzen.com" target="_blank">signalzen.com</a>.
    <br />
    In order to integrate with SignalZen and configure your setup, please click the button below, which will lead you to component configuration page.
    <br/>
    Once you enter your Public Token in the component configuration and save the configuration, your website will contain SignalZen live chat widget and you can start talking to them by using our Console or <a href="https://slack.com" target="_blank">Slack</a> if you choose to integrate with it also.
    <br />
    <br />
    <button onclick="window.location='index.php?option=com_config&view=component&component=com_signalzen'">Integrate and Configure SignalZen</button>
    <br />
    <br />
    Happy chatting!
  </p>
</div>
